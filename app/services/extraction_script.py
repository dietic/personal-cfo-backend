...
# This module contains the old bank-specific pattern matching logic.
# It has been replaced by AI-powered extraction (UniversalStatementService)
# but is kept as a fallback option.

# Feature flag to enable/disable legacy extraction
ENABLE_LEGACY_EXTRACTION = False  # Set to True to re-enable pattern-based extraction
...
def extract_transactions_from_statement(statement_text: str, bank_type: str = "BCP") -> List[Dict[str, Any]]:
    """
    LEGACY: Extract transactions from a bank statement based on the specified bank type.

    ⚠️  WARNING: This function is DEPRECATED and COMPLETELY DISABLED.
    ⚠️  Use UniversalStatementService with AI extraction instead.
    ⚠️  This function only works with BCP and DINERS banks.
    ...
    """
    # This function is completely disabled - any calls to it will immediately fail
    raise ValueError(
        "🚫 LEGACY EXTRACTION COMPLETELY DISABLED 🚫\n"
        f"Function: extract_transactions_from_statement(bank_type='{bank_type}')\n"
        "This legacy function has been completely disabled.\n"
        "Please use UniversalStatementService with AI extraction instead.\n"
        "Check the call stack to see what code is still calling this function."
    )
